...
# 塑性控制参数
yield_min = 0.95  # 最大压缩极限（越接近1越容易发生压缩塑性变形）
yield_max = 1.05  # 最大拉伸极限（越接近1越容易发生拉伸塑性变形）
...
def substep():
    ...
    for p in x:
        ...
        # 3. 塑性屈服截断：限制特征值在 [yield_min, yield_max] 之间
        ...
        if sdf < 0:
            ...
            ft_vector = ti.Vector.zero(float, 3) # 初始化切向摩擦力向量，根据你的库选择 vec(0) 或 np.zeros(3)等
            ...
            if v_t_norm > 1e-5: 
                ...
                # 库伦摩擦力限制：最大不能超过 mu_friction * f_n
