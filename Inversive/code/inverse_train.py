"""NN + Taichi AD inverse-physics training entry point.

This branch archives the original neural differentiable-simulation method and
its diagnostics.
"""
import argparse
import os
...
parser.add_argument("--train", action="store_true",
                    help="train the NN with Taichi AD through MPM")
parser.add_argument("--infer", action="store_true",
                    help="run inference with a trained NN")
parser.add_argument("--quick_test", action="store_true",
                    help="run 2 training epochs and exit")
...
def train():
    ...
    print(f"  seg_len = {seg_len}, segments = {num_segments}")
    print("-" * 72)
    print(f"{'epoch':>5} {'loss':>12} {'E':>10} {'|E-E*|':>10} "
          f"{'max|grad|':>12} {'time':>8}")
    print("-" * 72)

    for epoch in range(max_epochs):
        epoch_start = time.perf_counter()
        sim.init_particles()
        for field in grad_sensitive:
            sim.zero_grad(field)
        ...
        for seg in range(num_segments):
            ...
            epoch_loss += float(obs.loss[None])
        ...
        if epoch % 30 == 0 or epoch == max_epochs - 1:
            print(f"{epoch:5d} {epoch_loss:12.4e} {E_after:10.3f} "
